...
load("./rdaOutputs/addBinomVarDiff.rda")
...
write.table(MSE_out, "tables/selectBinomDiffMSE.txt", sep = "\t", row.names = F, quote = F)

## MSE table for all groups for supplementary
write.table(mutate(allMSE, MSE = round(MSE, 0)), "tables/allBinomDiffMSE.txt", sep = "\t", row.names = F, quote = F)
...
write.table(ER_out, "tables/selectBinomDiffER.txt", sep = "\t", row.names = F, quote = F)

## ER table for all groups for supplementary
write.table(mutate(allER, ER = round(ER, 0)), "tables/allBinomDiffER.txt", sep = "\t", row.names = F, quote = F)
