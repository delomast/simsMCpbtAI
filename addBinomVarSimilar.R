...
# this is one variable, similar between W and HNC of large scenario
...
for(sr in sampRate){
	...
	for(r in 1:numSims){
		...
		# similar between hatchery and wild, so sampling from same distribution for both
		varMat[,1] <- rbeta(nrow(varMat), 40, 60)
		varMat[,2] <- 1 - varMat[,1]
...
save(srSD_mean, srSD_upper, srSD_lower, srRec, trueComp, trueVarMat, convergeMLE, srMLE_mean,
	  srMLE_lower, srMLE_upper, srSD_tag100, srSD_spibetrFALSE, file = "./rdaOutputs/addBinomVarSimilar.rda")
