...
#set seed for R - used for data generation and scobi_deux
...
for(sr in sampRate){
	...
	for(r in 1:numSims){
		...
		aiRes <- estimStrataMCpbt(input, iter = 10000, burnIn = 1000, thin = 1, seed = 13)
		...
		sr_upper[currentRow,tempGroups] <- apply(escapeResults$totPiTotEstim[,tempGroups],2,quantile, .05)
		sr_lower[currentRow,tempGroups] <- apply(escapeResults$totPiTotEstim[,tempGroups],2,quantile, .95)
		...
		srSD_upper[currentRow,tempGroups] <- pbt_res[match(tempGroups, pbt_res[,1]), 3]
		srSD_lower[currentRow,tempGroups] <- pbt_res[match(tempGroups, pbt_res[,1]), 4]
		...
		srSD_upper[currentRow,tempGroups] <- wild_res[match(tempGroups, wild_res[,1]), 3]
		srSD_lower[currentRow,tempGroups] <- wild_res[match(tempGroups, wild_res[,1]), 4]
...
save(sr_mean, sr_upper, sr_lower, srSD_mean, srSD_upper, srSD_lower, srRec, trueComp, file = "./rdaOutputs/sampRateSimsOutput.rda")


# sr_mean[4,] - srSD_mean[4,]
# trueComp
# sum(sr_mean[1,] )
# 
# plot(escapeResults$totPiTotEstim[,"GSIgroup6"])
