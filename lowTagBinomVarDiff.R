...
for(sr in sampRate){
	...
	dataList <- list()
	for(r in 1:numSims){
		# add in some variable, low tag rates
		tagRates[,2] <- rbeta(nrow(tagRates), 15, 35)
...
save(srSD_mean, srSD_upper, srSD_lower, srRec, trueComp, trueVarMat, convergeMLE, srMLE_mean,
	  srMLE_lower, srMLE_upper, srSD_tag100, srSD_spibetrFALSE, file = "./rdaOutputs/lowTagBinomVarDiff.rda")
