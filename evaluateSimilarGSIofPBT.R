...
load("./rdaOutputs/similarGSIofPBT.rda")
...
write.table(MSE_out, "tables/selectSimilarGSIofPBTMSE.txt", sep = "\t", row.names = F, quote = F)

## MSE table for all groups for supplementary
write.table(mutate(allMSE, MSE = round(MSE, 0)), "tables/allSimilarGSIofPBTMSE.txt", sep = "\t", row.names = F, quote = F)
...
write.table(ER_out, "tables/selectSimilarGSIofPBTER.txt", sep = "\t", row.names = F, quote = F)

## ER table for all groups for supplementary
write.table(mutate(allER, ER = round(ER, 0)), "tables/allSimilarGSIofPBTER.txt", sep = "\t", row.names = F, quote = F)
