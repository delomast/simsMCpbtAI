...
compFunc <- function(data){
	...
	for(s in 1:length(input)){
		...
		input[[s]]$piTotPrior <- rep(1/length(input[[s]]$piTotPrior), length(input[[s]]$piTotPrior))
	}
...
save(sr_mean, sr_upper, sr_lower, srSD_mean, srSD_upper, srSD_lower, srRec, trueComp, convergeMLE, srMLE_mean,
	  srMLE_lower, srMLE_upper, file = "./rdaOutputs/large_sampRateSimsOutput_sym1divN.rda")
