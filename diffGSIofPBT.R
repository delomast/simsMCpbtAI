...
for(sr in sampRate){
	...
	dataList <- list()
	for(r in 1:numSims){
		# make all hatchery groups different and random in GSI composition
		gsiOfPbt <- t(replicate(25, ranDirich(c(1,1,20,1,25,1,30,1,20,1)[sample(1:10, 10, replace = FALSE)])))
...
save(srSD_mean, srSD_upper, srSD_lower, srRec, trueComp, convergeMLE, srMLE_mean,
	  srMLE_lower, srMLE_upper, srSD_tag100, srSD_spibetrFALSE, file = "./rdaOutputs/diffGSIofPBT.rda")
